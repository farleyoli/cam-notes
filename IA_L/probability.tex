...
\end{prop}
\begin{proof}
  \textbf{Hint: Bound $\log n!$ between left and right Riemann sums of $\log x$; squeeze.}
...
\begin{cor}[Stirling's asymptotic form]
  $n!\sim \sqrt{2\pi}\, n^{n + 1/2}\, e^{-n}$ as $n \to \infty$.
\end{cor}

\begin{proof}[Proof of Stirling's formula (non-examinable)]
  \textbf{Hint: Show $d_n = \log(n!\,e^n/n^{n+1/2})$ is decreasing and bounded below via $t = 1/(2n+1)$ substitution and Taylor series. Identify limit using Wallis-type integrals $I_n = \int_0^{\pi/2}\sin^n\theta\;\d\theta$.}
...
\begin{prop}[Stirling's sharp bounds]
  For all $n \geq 1$,
  \[
...
\begin{prop}[Elementary properties of probability]\leavevmode
  \begin{enumerate}
    \item $\P(\emptyset) = 0$.
...
\begin{proof}
  \textbf{Hint: Decompose into disjoint unions and apply additivity.}
\leavevmode
  \begin{enumerate}
...
\begin{proof}
  \textbf{Hint: Disjointify via $B_n = A_n \setminus A_{n-1}$; apply countable additivity. Decreasing case by complements.}
...
\begin{proof}
  \textbf{Hint: Disjointify; $B_i \subseteq A_i$ gives $\P(B_i) \leq \P(A_i)$.}
...
\begin{proof}
  \textbf{Hint: Induction; split off $A_1$ using the two-event formula.}
...
\begin{prop}[Independence and complements]
  If $A$ and $B$ are independent, then $A$ and $B^C$ are independent.
\end{prop}

\begin{proof}
  \textbf{Hint: $\P(A \cap B^C) = \P(A) - \P(A \cap B)$; factor out $\P(A)$.}
...
\begin{prop}[Independence in product spaces]
...
\begin{proof}
  \textbf{Hint: Double sum factorises into a product of single sums.}
...
\begin{proof}
  \textbf{Hint: Factor out $1/k!$; remaining factors $\to 1$, $\lambda^k$, and $e^{-\lambda}$ separately.}
...
\begin{prop}[Multiplication and chain rules]\leavevmode
  \begin{enumerate}
...
\begin{proof}
  \textbf{Hint: (1)--(3) from definition. (4): verify Kolmogorov axioms for $\P(\ph \mid B)$.}
...
\begin{prop}[Expectation calculus]\leavevmode
  \begin{enumerate}
...
\begin{proof}
  \textbf{Hint: Expand definition; (5): complete the square.}
\leavevmode
  \begin{enumerate}
...
\begin{proof}
  \textbf{Hint: Additivity plus induction.}
...
\begin{prop}[Variance calculus]\leavevmode
  \begin{enumerate}
...
\begin{proof}
  \textbf{Hint: Expand the square; use linearity.}
...
\begin{prop}[Indicator function algebra]
...
\begin{proof}
  \textbf{Hint: Check both cases $\omega \in A$ and $\omega \notin A$.}
...
\begin{thm}[Inclusion-exclusion, indicator proof]
...
\begin{proof}
  \textbf{Hint: Expand $1 - \prod(1 - I_j)$ and take expectations.}
...
\begin{prop}[Functions preserve independence]
...
\begin{proof}
  \textbf{Hint: Sum the joint pmf over preimages; the product factorises.}
...
\begin{thm}[Product rule for expectations]
...
\begin{proof}
  \textbf{Hint: Independence factorises the joint pmf; multi-sum becomes a product of single sums.}
...
\begin{cor}[Product rule for expectations, function form]
...
\begin{thm}[Sum rule for variances]
...
\begin{proof}
  \textbf{Hint: Expand the square; cross terms vanish by independence ($\E[X_i X_j] = \E[X_i]\E[X_j]$).}
...
\begin{proof}
  \textbf{Hint: Pull $1/n$ out as $1/n^2$; apply the sum rule; use identical distributions.}
...
\begin{prop}[Convexity criterion]
...
\begin{proof}
  \textbf{Hint: Induction. Group last $n-1$ points into one with weight $q = p_2 + \dotsb + p_n$; apply $n = 2$ case, then inductive hypothesis.}
...
\begin{proof}
  \textbf{Hint: Apply Jensen to $f(x) = -\log x$ with $p_i = 1/n$.}
...
\begin{proof}
  \textbf{Hint: Optimise $\E[(X - tY)^2] \geq 0$ over $t$; set $t = \E[XY]/\E[Y^2]$.}
...
\begin{proof}
  \textbf{Hint: Pointwise bound $I_{|X| \geq \varepsilon} \leq |X|/\varepsilon$; take expectations.}
...
\begin{proof}
  \textbf{Hint: Pointwise bound $I_{|X| \geq \varepsilon} \leq X^2/\varepsilon^2$; take expectations.}
...
\begin{cor}[Chebyshev's inequality, deviation form]
...
\begin{proof}
  \textbf{Hint: Apply Chebyshev to $X - \mu$.}
...
\begin{proof}
  \textbf{Hint: Chebyshev on $S_n/n$; variance is $\sigma^2/n \to 0$.}
...
\begin{prop}[Covariance calculus]
...
\begin{proof}
  \textbf{Hint: Expand $\E[(X - \E[X])(Y - \E[Y])]$; use linearity.}
...
\begin{prop}[Conditional expectation under independence]
  If $X$ and $Y$ are independent, then $\E[X\mid Y] = \E[X]$.
\end{prop}

\begin{proof}
  \textbf{Hint: Independence gives $\P(X = x \mid Y = y) = \P(X = x)$.}
...
\begin{proof}
  \textbf{Hint: Expand $\E[\E[X \mid Y]]$ as a double sum; swap order and marginalise.}
...
\begin{thm}[PGF uniqueness]
  The distribution of $X$ is uniquely determined by its pgf.
\end{thm}

\begin{proof}
  \textbf{Hint: Power series coefficients are unique.}
...
\begin{proof}
  \textbf{Hint: Differentiate term by term; monotone convergence as $z \to 1^-$.}
...
\begin{thm}[PGF addition formula]
...
\begin{proof}
  \textbf{Hint: $\E[z^{X_1 + \dotsb + X_n}]$ factorises by independence.}
...
\begin{proof}
  \textbf{Hint: Condition on $N$; given $N = n$, the pgf is $p(z)^n$.}
...
\begin{proof}
  \textbf{Hint: Chain rule on $h(p(z))$ at $z = 1$.}
...
\begin{proof}
  \textbf{Hint: Tower property; condition on $X_n = k$, then use independence of offspring.}
...
\begin{thm}[Branching process moments]
...
\begin{proof}
  \textbf{Hint: Mean: tower property gives $\E[X_n] = \mu\,\E[X_{n-1}]$; iterate. Variance: use $\E[X_n^2 \mid X_{n-1}] = X_{n-1}\sigma^2 + \mu^2 X_{n-1}^2$; set up recurrence and iterate.}
...
\begin{prop}[Extinction fixed-point equation]
  The extinction probability satisfies $F(q) = q$.
\end{prop}

\begin{proof}
  \textbf{Hint: $q = \lim F_n(0)$; use $F_{n+1} = F \circ F_n$ and continuity of $F$.}
...
\begin{proof}
  \textbf{Hint: Show $F_n(0) \leq \alpha$ by induction (monotonicity of $F$). For $\mu$ dichotomy: $F$ is convex with $F(1) = 1$, $F'(1) = \mu$.}
...
\begin{proof}
  \textbf{Hint: Condition on first step to get $p_z = p\,p_{z+1} + q\,p_{z-1}$. Try $p_z = t^z$; characteristic equation $pt^2 - t + q = 0$ has roots $1$ and $q/p$.}
...
\begin{proof}
  \textbf{Hint: Let $a \to \infty$ in $q_z$; use continuity of probability.}
...
\begin{proof}
  \textbf{Hint: Finiteness: probability of absorption in $a$ steps is $\geq p^a + q^a > 0$. Recurrence $D_z = 1 + p\,D_{z+1} + q\,D_{z-1}$; try particular solution $D_z = Cz$.}
...
\begin{proof}
  \textbf{Hint: Exponentials cancel: $e^{-\lambda(x+z)}/e^{-\lambda x} = e^{-\lambda z}$.}
...
\begin{proof}
  \textbf{Hint: Fubini: swap order of integration; inner integral $\int_0^y \d x = y$.}
...
\begin{prop}[Independence properties, continuous case]
...
\begin{proof}
  \textbf{Hint: Reduce to standard normal; compute $I^2$ via polar coordinates.}
...
\begin{prop}[Mean of the normal]
  If $X \sim N(\mu, \sigma^2)$, then $\E[X] = \mu$.
\end{prop}

\begin{proof}
  \textbf{Hint: Split $x = (x - \mu) + \mu$; odd integrand vanishes by symmetry.}
...
\begin{prop}[Variance of the normal]
  If $X \sim N(\mu, \sigma^2)$, then $\var(X) = \sigma^2$.
\end{prop}

\begin{proof}
  \textbf{Hint: Standardise to $Z$; integration by parts on $\int z^2 e^{-z^2/2}\;\d z$.}
...
\begin{thm}[Change of variables, one dimension]
...
\begin{proof}
  \textbf{Hint: Compute $F_Y(y) = F_X(h^{-1}(y))$; differentiate by chain rule.}
...
\begin{proof}
  \textbf{Hint: Apply $F$ to both sides: $\P(F^{-1}(U) \leq x) = \P(U \leq F(x)) = F(x)$.}
...
\begin{prop}[Change of variables, several dimensions]
...
\begin{proof}
  \textbf{Hint: Multivariable substitution; Jacobian accounts for volume distortion.}
...
\begin{proof}
  \textbf{Hint: (i)--(ii): all $X_i$ must be $\leq y$ (or $\geq y$). (iii): joint cdf, then differentiate. (iv): $n!$ orderings of iid variables.}
...
\begin{thm}[Moments from the MGF]
...
\begin{proof}
  \textbf{Hint: Taylor expand $e^{\theta X}$; take expectations term by term.}
...
\begin{thm}[MGF addition formula]
...
\begin{proof}
  \textbf{Hint: $e^{\theta(X+Y)} = e^{\theta X} e^{\theta Y}$; factorise by independence.}
...
\begin{prop}[Moments of the Cauchy distribution]
  The mean of the Cauchy distribution is undefined, and $\E[X^2] = \infty$.
\end{prop}

\begin{proof}
  \textbf{Hint: $\int_0^\infty x/(1+x^2)\;\d x$ diverges ($\sim \log$); both halves are $\pm\infty$.}
...
\begin{prop}[Stability of the Cauchy distribution]
...
\begin{proof}
  \textbf{Hint: Convolve the pdfs; partial fractions to evaluate the integral. Induction for general $n$.}
...
\begin{proof}
  \textbf{Hint: Compute mgf of $S_n$ by independence; match with $\Gamma(n, \lambda)$ mgf.}
...
\begin{prop}[Uniform order statistics and the beta distribution]
...
\begin{proof}
  \textbf{Hint: Standardise; complete the square in the exponent; recognise the remaining integral as a normal pdf.}
...
\begin{thm}[Closure of normal distributions]
...
\begin{proof}
  \textbf{Hint: Multiply mgfs (independence); exponents add. For scaling, replace $\theta$ by $a\theta$.}
\leavevmode
  \begin{enumerate}
    \item
      \begin{align*}
...
\begin{proof}
  \textbf{Hint: Multiply integrand by $1 \leq 1 + 1/t^2$; note $\frac{\d}{\d t}[\phi(t)/t] = -(1 + 1/t^2)\phi(t)$.}
...
If $X_1, \dotsc, X_n$ are iid $N(0, 1)$ and $\mathbf{x} = (x_1, \dotsc, x_n)^T$, then their joint density is
\begin{align*}
  g(x_1, \dotsc, x_n) &= \prod_{i = 1}^n \phi(x_i)
  = \frac{1}{(2\pi)^{n/2}} e^{-\frac{1}{2}\sum_1^n x_i^2}
  = \frac{1}{(2\pi)^{n/2}}e^{-\frac{1}{2}\mathbf{x}^T \mathbf{x}}.
\end{align*}

\begin{defi}[Multivariate normal]
  Let $\mathbf{X} = (X_1, \dotsc, X_n)^T$ with $X_i$ iid $N(0,1)$, let $A$ be an invertible $n \times n$ matrix, and let $\boldsymbol\mu \in \R^n$. The random vector $\mathbf{Z} = \boldsymbol\mu + A\mathbf{X}$ has the \emph{multivariate normal} distribution
  \[
    \mathbf{Z} \sim N(\boldsymbol\mu, \Sigma),
  \]
  where $\Sigma = AA^T$ is the \emph{covariance matrix}, with pdf
  \[
    f(\mathbf{z}) = \frac{1}{(2\pi)^{n/2}\sqrt{\det \Sigma}}\exp\left[-\frac{1}{2}(\mathbf{z} - \boldsymbol\mu)^T \Sigma^{-1}(\mathbf{z} - \boldsymbol\mu)\right].
  \]
\end{defi}

\begin{proof}[Derivation]
  \textbf{Hint: Change of variables $\mathbf{X} = A^{-1}(\mathbf{Z} - \boldsymbol\mu)$ with Jacobian $1/\det A$; use $\Sigma = AA^T$.}

  Since $\mathbf{X} = A^{-1}(\mathbf{Z} - \boldsymbol\mu)$ and $|J| = |\det(A^{-1})| = \frac{1}{\det A}$, we have
  \begin{align*}
    f(z_1, \dotsc, z_n) &= \frac{1}{(2\pi)^{n/2}\det A}\exp\left[-\frac{1}{2}(A^{-1}(\mathbf{z} - \boldsymbol\mu))^T(A^{-1}(\mathbf{z} - \boldsymbol\mu))\right]\\
    &= \frac{1}{(2\pi)^{n/2}\sqrt{\det \Sigma}}\exp\left[-\frac{1}{2}(\mathbf{z} - \boldsymbol\mu)^T \Sigma^{-1}(\mathbf{z} - \boldsymbol\mu)\right],
  \end{align*}
  where $\Sigma^{-1} = (A^{-1})^T A^{-1}$ and $\det \Sigma = (\det A)^2$.
\end{proof}

\begin{prop}[Covariance matrix of the multivariate normal]
  The matrix $\Sigma$ is the covariance matrix of $\mathbf{Z}$, i.e.\ $\Sigma_{ij} = \cov(Z_i, Z_j)$.
\end{prop}

\begin{proof}
  \textbf{Hint: $\E[A\mathbf{X}\mathbf{X}^T A^T] = A\,\E[\mathbf{X}\mathbf{X}^T]\,A^T = AI A^T = \Sigma$.}

  \begin{align*}
    \E[(\mathbf{Z} - \boldsymbol\mu)(\mathbf{Z} - \boldsymbol\mu)^T] &= \E[A\mathbf{X}(A\mathbf{X})^T]
    = A\,\E[\mathbf{X}\mathbf{X}^T]\,A^T
    = AIA^T
    = \Sigma.\qedhere
  \end{align*}
\end{proof}

\begin{prop}[Uncorrelated normal variables]
  If $Z_1, \dotsc, Z_n$ are jointly multivariate normal with $\cov(Z_i, Z_j) = 0$ for $i \neq j$, then $Z_1, \dotsc, Z_n$ are independent, with $Z_i \sim N(\mu_i, \sigma_i^2)$.
\end{prop}

\begin{proof}
  \textbf{Hint: $\Sigma$ diagonal $\Rightarrow$ joint pdf factorises into a product of univariate normal pdfs.}

  If the covariances are zero, then $\Sigma = \diag(\sigma_1^2, \dotsc, \sigma_n^2)$, so
  \[
    f(z_1, \dotsc, z_n) = \prod_{i=1}^n \frac{1}{\sqrt{2\pi}\,\sigma_i} e^{-\frac{1}{2\sigma_i^2}(z_i - \mu_i)^2}.\qedhere
  \]
\end{proof}

\begin{remark}
  Zero covariance implies independence for multivariate normal random variables, but not in general.
\end{remark}

\begin{defi}[Multivariate mgf]
  The \emph{moment generating function} of a random vector $\mathbf{X} = (X_1, \dotsc, X_n)^T$ is
  \[
    m(\boldsymbol\theta) = \E[e^{\boldsymbol\theta^T \mathbf{X}}] = \E[e^{\theta_1 X_1 + \dotsb + \theta_n X_n}].
  \]
\end{defi}

\subsubsection*{Bivariate normal}
In the case $n = 2$, the \emph{bivariate normal} has covariance matrix
\[
  \Sigma=
  \begin{pmatrix}
  ...
  \end{pmatrix},
\]
where $\rho = \mathrm{corr}(X_1, X_2)$. The joint pdf with mean $\boldsymbol\mu = \mathbf{0}$ is
\[
...
\]
For non-zero mean, replace $x_i$ with $x_i - \mu_i$. The joint mgf is
\[
...
\begin{thm}[Central limit theorem]
  Let $X_1, X_2, \dotsc$ be iid random variables with $\E[X_i] = \mu$ and $\var(X_i) = \sigma^2 < \infty$. Let $S_n = X_1 + \dotsb + X_n$. Then
  \[
...
\begin{thm}[Continuity theorem]
  If random variables $X_1, X_2, \dotsc$ have mgfs $m_1(\theta), m_2(\theta), \dotsc$ and $m_n(\theta) \to m(\theta)$ as $n\to\infty$ for all $\theta$, then $X_n \to_D$ the random variable with mgf $m(\theta)$.
\end{thm}

\begin{proof}[Sketch proof of CLT]
  \textbf{Hint: WLOG $\mu = 0$, $\sigma^2 = 1$. Taylor expand mgf of $X_i$; raise to the $n$th power with $\theta/\sqrt{n}$; use $(1 + a/n)^n \to e^a$. Apply continuity theorem.}

  Without loss of generality, assume $\mu = 0$ and $\sigma^2 = 1$ (otherwise replace $X_i$ with $\frac{X_i - \mu}{\sigma}$). Then
  \begin{align*}
    m_{X_i}(\theta) &= \E[e^{\theta X_i}] = 1 + \theta \E[X_i] + \frac{\theta^2}{2!} \E[X_i^2] + \dotsb\\
    &= 1 + \frac{1}{2}\theta^2 + \frac{1}{3!}\theta^3 \E[X_i^3] + \dotsb
  \end{align*}
  The mgf of $S_n/\sqrt{n}$ is
  \begin{align*}
    \E[e^{\theta S_n/\sqrt{n}}] &= \E[e^{\theta(X_1 + \dotsb + X_n)/\sqrt{n}}]\\
    &= \left(\E[e^{\theta X_1/\sqrt{n}}]\right)^n\\
    &= \left(1 + \frac{1}{2}\theta^2 \frac{1}{n} + \frac{1}{3!}\theta^3 \E[X^3]\frac{1}{n^{3/2}} + \dotsb\right)^{n}\\
    &\to e^{\frac{1}{2}\theta^2}
  \end{align*}
  as $n \to \infty$, since $(1 + a/n)^n \to e^a$. This is the mgf of $N(0,1)$, so the result follows from the continuity theorem.
\end{proof}

\begin{remark}
  This sketch proof requires $\E[X^3]$ to be finite. The proper proof uses the \emph{characteristic function} $\chi_X(\theta) = \E[e^{i\theta X}]$, which is always defined.
\end{remark}

\begin{eg}[Normal approximation to the binomial]
  Let $X_i \sim B(1, p)$. Then $S_n \sim B(n, p)$, and the CLT gives
  \[
...
\begin{eg}
  ...
  \[
    F(s) = \P(S > s),
  \]
  i.e.\ the probability that plane 1 is overbooked? We have
  \[
    F(s) = \P(S > s) = \P\left(\frac{S - n/2}{\sqrt{n}/2} > \frac{s - n/2}{\sqrt{n}/2}\right).
  \]
  By the CLT, $\frac{S - n/2}{\sqrt{n}/2} \to_D N(0, 1)$, so
  \[
...
\begin{eg}
  ...
  \]
  We use $0.975$ instead of $0.95$ since this is a two-tailed test. Since the maximum possible value of $p(1 - p)$ is $1/4$, we have
  \[
    n \geq 38416.
  \]
...
\begin{eg}[Estimating $\pi$ with Buffon's needle]
  ...
  \begin{align*}
    ...
    &= \frac{\pi p}{p + (p' - p)}\\
    &= \pi \left(1 - \frac{p' - p}{p} + \dotsb \right)
  \end{align*}
  Hence
  \[
...
\begin{center}
  \begin{tabular}{ccccc}
    ...
    Geometric & $(1 - p)^k p$ & $\displaystyle\frac{1 - p}{p}$ & $\displaystyle\frac{1 - p}{p^2}$ & $\displaystyle\frac{p}{1 - (1-p)z}$\\
