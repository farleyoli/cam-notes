...
\begin{defi}[Limits involving vectors]
  Let $\mathbf{v}$ and $\mathbf{c}$ be vectors in $\R^n$. We write $\mathbf{v}\to \mathbf{c}$ if $|\mathbf{v} - \mathbf{c}| \to 0$. For a scalar-valued function $f$, we write $f(\mathbf{r}) = o(\mathbf{r})$ to mean $\frac{|f(\mathbf{r})|}{|\mathbf{r}|} \to 0$ as $\mathbf{r}\to \mathbf{0}$.
\end{defi}
...
\begin{defi}[Smooth function]
  A function is \emph{smooth} if all partial derivatives of all orders exist and are continuous. By Schwarz's theorem, this implies that mixed partial derivatives are symmetric, i.e.\ the order of differentiation does not matter.
\end{defi}
...
\begin{rmk}[Basis vectors for curvilinear coordinates]
  The coordinates $\rho$ and $\varphi$ are not the components of the position vector in any basis; that is, we do not have $\mathbf{r} = \rho\,\mathbf{e}_\rho + \varphi\,\mathbf{e}_\varphi$ in the same way that $\mathbf{r} = x_1\mathbf{e}_1 + x_2\mathbf{e}_2$. Nevertheless, we can define associated basis vectors that point in the directions of increasing $\rho$ and $\varphi$, obtained by differentiating the position vector with respect to each coordinate and normalizing:
  \[
...
\begin{rmk}[Reparametrisation]
  ...
  \]
  A particularly convenient choice is to use the arc length $s$ itself as the parameter. In this case, the tangent vector has unit length, since by the arclength derivative proposition,
  \[
...
Just as matrices correspond to linear maps, tensors correspond to multi-linear maps.
...
\begin{rmk}
  This establishes a one-to-one correspondence between rank-$n$ tensors and multi-linear maps from $n$ vectors to $\R$. This viewpoint defines tensors independently of any coordinate system; the transformation rule then emerges as a consequence. The same approach relates matrices to linear maps.
\end{rmk}
...
Assuming sufficient smoothness, tensor fields can be differentiated:
...
Tensor-valued integrals are defined component-wise. For a tensor field $T_{ij\cdots k}(\mathbf{x})$, the integral $\int_V T_{ij\cdots k}\,\d V$ is a tensor of the same rank.

\begin{eg}[Momentum flux tensor]
  For a fluid with velocity $\mathbf{u}(\mathbf{x})$ and density $\rho$, the flux of the $i$th component of momentum through a surface element is $\rho u_i u_j n_j \,\delta S = T_{ij}n_j \,\delta S$, where $T_{ij} = \rho u_iu_j$ is the momentum flux tensor.
\end{eg}

The divergence theorem generalizes to tensor fields. Let $V$ be a volume bounded by surface $S = \partial V$, and let $T_{ij\cdots k\ell}$ be a smooth tensor field.
...
Since the antisymmetric part has 3 independent components, it can be written in terms of a vector. Indeed,
\[
  A_{ij} = \varepsilon_{ijk}B_k
\]
for some vector $\mathbf{B}$. Contracting both sides with $\varepsilon_{ij\ell}$ and using $\varepsilon_{ijk}\varepsilon_{ij\ell} = 2\delta_{k\ell}$ gives
\[
  B_k = \frac{1}{2}\varepsilon_{ijk}A_{ij} = \frac{1}{2}\varepsilon_{ijk}T_{ij},
\]
where the last equality holds because the symmetric part of $T_{ij}$ does not contribute to the contraction with $\varepsilon_{ijk}$.
...
Consider point masses $m_\alpha$ at positions $\mathbf{r}_\alpha$, rotating with angular velocity $\boldsymbol\omega$ about the origin. The velocities are $\mathbf{v}_\alpha = \boldsymbol\omega\times \mathbf{r}_\alpha$, and the total angular momentum is
\begin{align*}
...
\end{align*}
using the vector identity $\mathbf{a}\times(\mathbf{b}\times\mathbf{a}) = |\mathbf{a}|^2\mathbf{b} - (\mathbf{a}\cdot\mathbf{b})\mathbf{a}$. In components,
\[
  L_i = I_{ij}\omega_j,
\]
where
...
\begin{eg}[Invariant integral for sphere]
  ...
  A closely related calculation gives the inertia tensor of a solid sphere of constant density $\rho_0$, or of mass $M = \frac{4}{3}\pi a^3 \rho_0$.
