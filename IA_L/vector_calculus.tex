...
\begin{proof}
  Note that $\Phi$ maps from $(u, v)$ to $(x, y)$. Consider a small rectangle in the $(u, v)$-plane with sides $\delta u$ and $\delta v$. By Taylor's theorem,
  \[
    \delta x = x(u + \delta u, v + \delta v) - x(u, v) \approx \frac{\partial x}{\partial u}\delta u + \frac{\partial x}{\partial v}\delta v,
  \]
  and similarly for $\delta y$. In matrix form,
  \[
    \begin{pmatrix}
      \delta x\\
      \delta y
    ...
    \end{pmatrix}.
  \]
  Since a linear map scales areas by the absolute value of its determinant, the area element $\delta u\,\delta v$ is mapped to $|J|\,\delta u\,\delta v$. Hence
  \[
...
\begin{eg}[Polar area element derivation]
  ...
  \]
  Suppose we want to integrate a function over the quarter disc $D$ of radius $R$ in the first quadrant.
  \begin{center}
    \begin{tikzpicture}
  ...
  \end{align*}
  Note that the integral exists as $R\to \infty$, giving $\frac{\pi}{2}$.

  Now we evaluate the same integral over the first quadrant in Cartesian coordinates. Since $f$ is separable,
  \begin{align*}
    \int_{x = 0}^{\infty} \int_{y=0}^\infty e^{-(x^2 + y^2)/2}\;\d x\;\d y
    &= \left(\int_0^\infty e^{-x^2/2}\;\d x\right)\left(\int_0^\infty e^{-y^2/2}\;\d y\right)
    = \frac{\pi}{2},
  \end{align*}
  where the last equality follows from ($*$). So each factor must equal $\sqrt{\pi/2}$, i.e.
  \[
...
\begin{eg}[Spherical symmetry volume integral]
  ...
  \end{align*}
  where in the second line we placed the differentials next to the integral signs, with the limits now implicit in the order of integration. The integral separates since $f$ depends only on $r$.

  This is a useful general result: the integral of a spherically symmetric function over a ball reduces to a one-dimensional integral, which can be interpreted as summing over spherical shells of thickness $\delta r$ and volume $4\pi r^2 \delta r$.

  Setting $f = 1$ gives the volume of a sphere as $\frac{4}{3}\pi a^3$.
\end{eg}

\begin{eg}[Sphere with cylinder removed]
  Consider the region within a sphere of radius $a$ with a cylinder of radius $b$ ($b < a$) removed, defined by
  \begin{align*}
    x^2 + y^2 + z^2 &\leq a^2\\
    x^2 + y^2 &\geq b^2.
  \end{align*}
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  We use cylindrical coordinates. The second criterion gives
  \[
    b \leq \rho \leq a.
  \]
...
\begin{eg}[Hemisphere total charge]
  ...
  In spherical polar coordinates, the region is described by
  \[
...
\subsubsection*{Integration in \tph{$\R^n$}{Rn}{&\#x211D;<sup>n</sup>}}
...
\begin{eg}[Hemisphere center of mass]
  ...
  \]
  Writing $\mathbf{R} = (X, Y, Z)$, by symmetry we expect $X = Y = 0$. To verify,
  \begin{align*}
    X &= \frac{1}{M}\int_H x\rho \;\d V\\
    &= \frac{\rho}{M}\int_0^a \int_0^{\pi/2}\int_0^{2\pi}xr^2 \sin \theta\;\d \varphi\;\d \theta\;\d r\\
    &= \frac{\rho}{M}\int_0^{a}r^3\;\d r \int_0^{\pi/2}\sin^2\theta\;\d \theta \int_0^{2\pi}\cos\varphi\;\d \varphi\\
    &= 0,
  \end{align*}
  since $\int_0^{2\pi}\cos\varphi\;\d\varphi = 0$. Similarly, $Y = 0$.
...
\subsection{Surfaces and normals}
A surface in $\R^3$ can be specified implicitly by an equation $f(\mathbf{r}) = c$, where $f: \R^3 \to \R$ is smooth and $c$ is a constant.
...
\begin{rmk}
  To see this, consider any curve $\mathbf{r}(u)$ lying on $S$. Since $f(\mathbf{r}(u)) = c$, differentiating gives
  \[
    \nabla f \cdot \frac{\d\mathbf{r}}{\d u} = 0,
  \]
  so $\nabla f$ is perpendicular to every tangent vector on $S$.
\end{rmk}
...
\begin{eg}[Level surfaces: sphere and hyperboloid]\leavevmode
  \begin{enumerate}
    ...
    \item Take $f(\mathbf{r}) = x^2 + y^2 - z^2 = c$, which is a hyperboloid. Then $\nabla f = 2(x, y, -z)$.
      ...
      In the special case $c = 0$, we have a double cone with a singularity at the apex $\mathbf{0}$. Here $\nabla f = \mathbf{0}$, and we cannot define a meaningful normal direction.
  \end{enumerate}
\end{eg}
...
\begin{defi}[Parametrized surface]
  A \emph{parametrization} of a surface $S$ is a smooth function $\mathbf{r}: D \to \R^3$, where $D \subseteq \R^2$ is a region in the $(u, v)$-plane, such that the image of $\mathbf{r}$ is $S$. Each point on $S$ is then labelled by coordinates $(u, v)$.
\end{defi}
...
\begin{eg}[Spherical cap parametrization]
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  We can then label the points on the sphere by the angles $\theta, \varphi$, with
  \[
    \mathbf{r}(\theta, \varphi) = (a\sin \theta\cos\varphi, a\sin \theta\sin \varphi, a\cos \theta) = a\mathbf{e}_r.
  \]
...
\begin{defi}[Regular parametrization]
  The partial derivatives $\frac{\partial \mathbf{r}}{\partial u}$ and $\frac{\partial \mathbf{r}}{\partial v}$ are tangent vectors to the coordinate curves on $S$. A parametrization is called \emph{regular} if
  \[
    \frac{\partial \mathbf{r}}{\partial u} \times \frac{\partial \mathbf{r}}{\partial v} \neq \mathbf{0}
  \]
  at every point $(u, v) \in D$, i.e., the tangent vectors are linearly independent everywhere. All parametrizations in this course are assumed to be regular.
\end{defi}
...
\begin{rmk}
  A small rectangle with sides $\delta u$, $\delta v$ in parameter space maps to a parallelogram on $S$ with sides approximately $\frac{\partial \mathbf{r}}{\partial u} \delta u$ and $\frac{\partial \mathbf{r}}{\partial v} \delta v$. Its area is $\left|\frac{\partial \mathbf{r}}{\partial u} \times \frac{\partial \mathbf{r}}{\partial v}\right| \delta u\, \delta v$, giving the scalar area element. The cross product itself gives the vector area element $\d\mathbf{S} = \mathbf{n}\,\d S$, where the ordering of $u$ and $v$ determines the orientation. The total area of $S$ is
  \[
...
\begin{eg}[Spherical cap scalar area]
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  The parametrization is $\mathbf{r}(\theta, \varphi) = (a\sin \theta\cos\varphi, a\sin \theta\sin \varphi, a\cos \theta) = a\mathbf{e}_r$, so
  \[
  ...
  and hence
  \[
    \frac{\partial \mathbf{r}}{\partial \theta}\times \frac{\partial \mathbf{r}}{\partial \varphi} = a^2\sin \theta\, \mathbf{e}_r, \quad \d S = a^2\sin \theta\;\d \theta\;\d \varphi.
  \]
  With bounds $0 \leq \theta \leq \alpha$ and $0 \leq \varphi \leq 2\pi$, the area is
  \[
...
\begin{rmk}
  We integrate the normal component $\mathbf{F} \cdot \mathbf{n}$ rather than $|\mathbf{F}|$: if $\mathbf{F}$ is tangent to $S$ everywhere, nothing crosses the surface, yet $|\mathbf{F}|$ may be large.
\end{rmk}
...
\begin{eg}[Sphere vector area element]
  Consider a sphere of radius $a$ parametrized by $\mathbf{r}(\theta, \varphi)$. Then
  \[
    \frac{\partial \mathbf{r}}{\partial \theta} = a\mathbf{e}_\theta,\quad \frac{\partial \mathbf{r}}{\partial \varphi} = a\sin \theta \mathbf{e}_\varphi.
  \]
  ...
  Suppose we want to calculate the fluid flux through the surface. The \emph{velocity field} $\mathbf{u}(\mathbf{r})$ gives the velocity of the fluid at position $\mathbf{r}$. For a small area element $\delta \mathbf{S}$ on a surface $S$, the volume of fluid crossing it in time $\delta t$ is $\mathbf{u}\cdot \delta \mathbf{S}\; \delta t$.
  \begin{center}
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
  \end{center}
  So the total volume of fluid crossing $S$ in time $\delta t$ is
  \[
    \delta t\int_S \mathbf{u}\cdot \d \mathbf{S}.
  \]
  So $\int_S \mathbf{u}\cdot \d \mathbf{S}$ is the \emph{rate} of volume crossing $S$.

  For example, let $\mathbf{u} = (-x, 0, z)$ and $S$ be the spherical cap with $0 \leq \theta \leq \alpha$, $0 \leq \varphi \leq 2\pi$. With outward normal $\mathbf{n} = \mathbf{r}/a$,
  \[
    \mathbf{n}\cdot \mathbf{u} = \frac{1}{a}(-x^2 + z^2) = a(\cos^2 \theta - \sin^2\theta\cos^2\varphi).
  \]
  Therefore
  \begin{align*}
...
\begin{rmk}
  To see this, let $\mathbf{r}(u, v)$ and $\mathbf{r}(\tilde{u}, \tilde{v})$ be two regular parametrizations. By the chain rule,
  \[
  ...
  \]
  where $\frac{\partial (\tilde{u}, \tilde{v})}{\partial(u, v)}$ is the Jacobian. Since $\d \tilde{u}\;\d \tilde{v} = \frac{\partial (\tilde{u}, \tilde{v})}{\partial (u, v)}\;\d u\;\d v$, the Jacobian factors cancel.
\end{rmk}
...
\begin{prop}[Change of variables in $\R^2$]
  If $(x, y) = (x(u, v), y(u, v))$ maps a region $D$ in the $(u, v)$-plane to a region $S$ in the $(x, y)$-plane, then
  \[
    \int_S f(x, y)\;\d S = \int_D f(x(u, v), y(u, v)) \left|\frac{\partial(x, y)}{\partial(u, v)}\right|\;\d u\;\d v.
  \]
\end{prop}

\begin{rmk}
  This follows from the surface area formula by embedding $S$ in $\R^3$ via $\mathbf{r}(u, v) = (x(u, v), y(u, v), 0)$. Then $\frac{\partial \mathbf{r}}{\partial u}\times \frac{\partial\mathbf{r}}{\partial v} = (0, 0, J)$, where $J = \frac{\partial(x, y)}{\partial(u, v)}$, so the scalar area element is $|J|\;\d u\;\d v$.
\end{rmk}

\subsubsection*{Change of variable formula in \tph{$\R^3$}{R3}{&\#x211D;<sup>3</sup>}}

\begin{prop}[Change of variables in $\R^3$]
  If $(x, y, z) = \mathbf{r}(u, v, w)$ maps a region $D$ to a region $V$, then
  \[
...
\begin{rmk}
  A small cuboid $\delta u\, \delta v\, \delta w$ in parameter space maps to a parallelepiped with edges $\frac{\partial \mathbf{r}}{\partial u}\delta u$, $\frac{\partial \mathbf{r}}{\partial v}\delta v$, $\frac{\partial \mathbf{r}}{\partial w}\delta w$. Its volume is the absolute value of the scalar triple product:
  \[
...
\begin{rmk}
  Let $\mathbf{r}(s)$ be a curve parametrized by arclength $s$, with unit tangent $\mathbf{t}(s) = \frac{\d \mathbf{r}}{\d s}$. Since $\mathbf{t}\cdot \mathbf{t} = 1$, differentiating gives $\mathbf{t}\cdot \mathbf{t}' = 0$, so $\mathbf{t}'$ is perpendicular to $\mathbf{t}$ whenever $\mathbf{t}' \neq \mathbf{0}$.
\end{rmk}
...
\begin{rmk}
  Taking $s = 0$ at the point of interest, the Taylor expansion of the curve is
  \[
    \mathbf{r}(s) = \mathbf{r}(0) + s\mathbf{t} + \tfrac{1}{2}\kappa s^2 \mathbf{n} + O(s^3),
  \]
  since $\mathbf{r}'' = \mathbf{t}' = \kappa \mathbf{n}$. A circle of radius $a$ through $\mathbf{r}(0)$ in the $(\mathbf{t}, \mathbf{n})$-plane has parametric equation $\mathbf{r} = \mathbf{r}(0) + a(1 - \cos \theta) \mathbf{n} + a\sin \theta\, \mathbf{t}$.
  \begin{center}
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
  \end{center}
  With arclength $s = a\theta$, expanding gives
  \[
    \mathbf{r} = \mathbf{r}(0) + s\mathbf{t} + \frac{1}{2a}s^2\mathbf{n} + O(s^3).
  \]
  Comparing coefficients of $s^2$ yields $\kappa = 1/a$.
\end{rmk}

\subsection{The Frenet--Serret frame}

\begin{defi}[Binormal]
  In three dimensions, the vectors $\mathbf{t}(s)$ and $\mathbf{n}(s)$ can be completed to an orthonormal basis. The \emph{binormal} of a curve is the unit vector $\mathbf{b} = \mathbf{t}\times \mathbf{n}$.
\end{defi}

\begin{defi}[Frenet--Serret frame]
  The ordered triple $(\mathbf{t}, \mathbf{n}, \mathbf{b})$ forms a right-handed orthonormal basis at each point of the curve, called the \emph{Frenet--Serret frame}.
\end{defi}
...
\begin{rmk}
  The geometry of a space curve is completely determined by how the Frenet--Serret frame evolves along the curve. The curvature $\kappa(s)$ measures how the curve bends in the osculating plane, and the torsion $\tau(s)$ measures how it twists out of that plane.
\end{rmk}

\subsection{Curvature of surfaces*}

\begin{rmk}
  At a point $P$ on a surface $S$ with unit normal $\mathbf{n}$, any plane containing $\mathbf{n}$ intersects $S$ in a curve through $P$, and this curve has some curvature $\kappa$ at $P$. Different choices of plane yield different curvatures.
\end{rmk}
...
\begin{defi}[Geodesic triangle]
  A \emph{geodesic triangle} $D$ on a surface $S$ is a region bounded by three geodesics. Let $\theta_1, \theta_2, \theta_3$ denote its interior angles.
\end{defi}
...
\begin{defi}[Nabla operator]
  In Cartesian coordinates with right-handed orthonormal basis $\{\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3\}$ (so $\mathbf{e}_i\times \mathbf{e}_j = \varepsilon_{ijk} \mathbf{e}_k$), the \emph{nabla} (or \emph{del}) operator is the formal vector
  \[
  ...
  \]
  The gradient of a scalar field $f$ is $\nabla f$, with components $(\nabla f)_i = \frac{\partial f}{\partial x_i}$. Applying $\nabla$ to a vector field $\mathbf{F}(\mathbf{r}) = F_i(\mathbf{r})\mathbf{e}_i$ via the scalar or vector product gives the divergence and curl.
\end{defi}
...
\begin{proof}
  ...
  \]
  Dividing by $\vol(V)$ and taking the limit as $V$ shrinks to $\mathbf{r}_0$ yields the result.
\end{proof}
...
\begin{proof}
  Applying Stokes' theorem to a small surface $A$ containing $\mathbf{r}_0$ with unit normal $\mathbf{n}$,
  \[
  ...
  \]
  Dividing by $\area(A)$ and taking the limit as $\mathrm{diam}(A) \to 0$ yields the result.
\end{proof}
