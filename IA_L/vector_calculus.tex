...
\begin{align*}
  \frac{\d \mathbf{r}}{\d u} &= \frac{\d \mathbf{r}}{\d \tilde{u}} \frac{\d \tilde{u}}{\d u}\\
  \frac{\d \mathbf{r}}{\d \tilde{u}} &= \frac{\d \mathbf{r}}{\d u}\bigg/ \frac{\d \tilde{u}}{\d u}
\end{align*}
...
\begin{prop}[Arclength differential]
...
\begin{prop}[Exactness symmetry test]
...
\begin{prop}[Differential linearity and product rule]
  \begin{align*}
...
\begin{prop}[Change of variables, area integrals]
...
\begin{prop}[Change of variables, volume integrals]
  \[
...
\begin{prop}[Change of variables, n-dimensional]
  \[
...
\]
Then $\frac{\partial \mathbf{r}}{\partial u}$ and $\frac{\partial \mathbf{r}}{\partial v}$ are tangent vectors to curves on $S$ with $v$ and $u$ constant respectively. What we want is for them to point in different directions.
...
We take a curve that can be Taylor expanded around $s = 0$. Then
\[
...
\end{thm}
This is the start of \emph{intrinsic geometry}: if we embed a surface in Euclidean space, we can determine lengths, angles etc on it. But we don't have to do so --- we can ``live in '' the surface and do geometry in it without an embedding.
...
\begin{prop}[Leibniz rules for vector operators]
  We have the following Leibniz properties:
  \begin{align*}
    \nabla(fg) &= (\nabla f)g + f(\nabla g)\\\
...
Note that this is true only if $\mathbf{F}$ or $\mathbf{H}$ is defined on all of $\R^3$.
...
There are three big integral theorems, known as Green's theorem, Stoke's theorem and Gauss' theorem. These are all generalizations of the fundamental theorem of calculus in some sense. In particular, they all say that an $n$ dimensional integral of a derivative is equivalent to an $n - 1$ dimensional integral of the original function.
...
\begin{prop}[Stokes' implies Green's]
...
\begin{prop}[Green's implies Stokes']
...
\begin{prop}[Green's vs 2D divergence theorem]
...
\begin{prop}[Divergence limit definition]
  \[
...
\]
So
\begin{prop}[Curl limit definition]
  \[
...
\begin{eg}[Fluid flow interpretation of div and curl]
...
\begin{eg}[Multi-valued potential, non-simply-connected domain]
  Take
  \[
...
\[
  \d \mathbf{r} = \frac{\partial \mathbf{r}}{\partial u}\;\d u + \frac{\partial \mathbf{r}}{\partial v}\;\d v + \frac{\partial \mathbf{r}}{\partial w}\;\d w = h_u \mathbf{e}_u \;\d u + h_v \mathbf{e}_v \;\d v + h_w \mathbf{e}_w \;\d w,
\]
...
\begin{prop}[Nabla in curvilinear coordinates]
  \[
...
\begin{prop}[Curl and divergence in curvilinear coordinates]
  \begin{align*}
...
\begin{proof}(non-examinable)
  \begin{enumerate}
    ...
    \item Use the integral expressions for div and curl.
      ...
      \]
      So, by the integral definition of curl,
      \[
...
The condition $\int_C \mathbf{g}\cdot \d \mathbf{r} = 0$ for any closed $C$ can be re-written by Stokes' theorem as
\[
...
\[
  \int_V \nabla^2\varphi \;\d V = \int_S \frac{\partial \varphi}{\partial \mathbf{n}}\;\d S.
\]
