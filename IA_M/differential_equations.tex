...
This chapter provides a brief review of differentiation and integration, with particular emphasis on the asymptotic notation that will be used throughout this course. We assume the reader is familiar with the basic properties of limits and continuous functions; a rigorous treatment of these concepts may be found in Analysis~I.

\subsection{Definition and first properties}
We begin by recalling the notion of the derivative of a function.

\begin{defi}[Derivative]
  Let $I \subset \R$ be an open interval, let $f \colon I \to \R$ be a function, and let $x \in I$. We say that $f$ is \emph{differentiable} at $x$ if the limit
  \[
    \lim_{h\to 0} \frac{f(x + h) - f(x)}{h}
  \]
  exists. When this limit exists, its value is called the \emph{derivative} of $f$ at $x$, denoted by $f'(x)$ or $\dfrac{\d f}{\d x}(x)$. We say that $f$ is \emph{differentiable on $I$} if it is differentiable at every point of $I$.
\end{defi}

\begin{notation}
  Several notations for derivatives are in common use:
  \begin{itemize}
    \item \emph{Lagrange's notation}: $f'(x)$ for the first derivative, $f''(x)$ for the second, and more generally $f^{(n)}(x)$ for the $n$-th derivative.
    \item \emph{Leibniz's notation}: $\dfrac{\d f}{\d x}$ or $\dfrac{\d}{\d x}f(x)$.
  \end{itemize}
  Higher derivatives are written as $\dfrac{\d^2 f}{\d x^2}$, $\dfrac{\d^3 f}{\d x^3}$, and so on. Note that $f'$ denotes the derivative with respect to the argument of $f$: for instance, if $g(x) = f(2x)$, then $g'(x) = 2f'(2x)$.
\end{notation}

\begin{eg}[Non-differentiability of the absolute value]
  Consider the absolute value function $f \colon \R \to \R$ defined by $f(x) = |x|$. This function is not differentiable at $x = 0$. Indeed, we have
  \[
    \lim_{h\to 0^+} \frac{|h| - |0|}{h} = 1 \quad \text{whereas} \quad \lim_{h\to 0^-} \frac{|h| - |0|}{h} = -1,
  \]
  so the limit does not exist.
\end{eg}

\subsection{Asymptotic notation: little-\texorpdfstring{$o$}{o} and big-\texorpdfstring{$O$}{O}}
When studying the behaviour of functions near a point or at infinity, it is convenient to have notation for comparing the relative sizes of functions. The following definitions introduce the \emph{Landau notation}, which is used extensively in analysis.

\begin{defi}[Little-$o$ notation]
  Let $f$ and $g$ be real-valued functions defined in a neighbourhood of a point $x_0 \in \R \cup \{-\infty, +\infty\}$, with $g(x) \neq 0$ for $x$ sufficiently close to $x_0$ (but $x \neq x_0$). We write
  \[
    f(x) = o(g(x)) \quad \text{as } x \to x_0
  \]
  if $\displaystyle \lim_{x\to x_0} \frac{f(x)}{g(x)} = 0$. Informally, this means that $f(x)$ becomes negligible compared to $g(x)$ as $x$ approaches $x_0$.
\end{defi}

\begin{defi}[Big-$O$ notation]
  Let $f$ and $g$ be real-valued functions defined in a neighbourhood of $x_0 \in \R \cup \{-\infty, +\infty\}$, with $g(x) \neq 0$ for $x$ sufficiently close to $x_0$. We write
  \[
    f(x) = O(g(x)) \quad \text{as } x \to x_0
  \]
  if there exist constants $M > 0$ and $\delta > 0$ such that $|f(x)| \leqslant M|g(x)|$ for all $x$ with $0 < |x - x_0| < \delta$. Note that for this condition to hold, the limit $\lim_{x\to x_0} \frac{f(x)}{g(x)}$ need not exist.
\end{defi}

\begin{rem}
  The notation $f(x) = o(g(x))$ is, strictly speaking, an abuse of notation: the symbol $o(g(x))$ does not represent a specific function, but rather a \emph{class} of functions satisfying the condition above. The statement $f(x) = o(g(x))$ should be read as ``$f$ belongs to the class $o(g)$''. Similar remarks apply to big-$O$ notation. Observe that $f(x) = o(g(x))$ implies $f(x) = O(g(x))$, but the converse is false in general.
\end{rem}

\begin{eg}[Asymptotic comparisons]\leavevmode
  \begin{enumerate}
    \item As $x \to 0$: we have $x = o(\sqrt{x})$, since $\lim_{x \to 0} \frac{x}{\sqrt{x}} = \lim_{x \to 0} \sqrt{x} = 0$.
    \item As $x \to +\infty$: we have $\sqrt{x} = o(x)$, since $\lim_{x \to +\infty} \frac{\sqrt{x}}{x} = \lim_{x \to +\infty} \frac{1}{\sqrt{x}} = 0$.
    \item As $x \to 0$: we have $\sin 2x = O(x)$. Indeed, $\lim_{x \to 0} \frac{\sin 2x}{x} = 2$.
    \item As $x \to +\infty$: we have $\sin 2x = O(1)$, since $|\sin 2x| \leqslant 1$ for all $x$, even though $\lim_{x \to +\infty} \sin 2x$ does not exist.
  \end{enumerate}
\end{eg}

This notation is particularly useful for expressing that certain terms are negligible in approximations. For instance, writing $f(x) = g(x) + O(h^2)$ indicates that the error in approximating $f$ by $g$ is bounded by a constant multiple of $h^2$.

The following proposition provides an important characterisation of differentiability in terms of little-$o$ notation.
\begin{prop}[Linear approximation]
  Let $I \subset \R$ be an open interval and let $f \colon I \to \R$ be differentiable at a point $x_0 \in I$. Then
  \[
...
\begin{proof}
  ...
  \]
  Setting $\varepsilon(h) = \frac{f(x_0 + h) - f(x_0)}{h} - f'(x_0)$, we see that $\varepsilon(h) \to 0$ as $h \to 0$. Hence
  \[
    f(x_0 + h) = f(x_0) + f'(x_0)\, h + h\, \varepsilon(h),
  \]
  where $h\, \varepsilon(h) = o(h)$ as $h \to 0$.
\end{proof}

\subsection{Rules of differentiation}
We now recall the principal rules for computing derivatives. These rules, combined with knowledge of the derivatives of elementary functions, allow us to differentiate a wide variety of expressions.

\begin{thm}[Chain rule]
  Let $I, J \subset \R$ be open intervals. Let $g \colon I \to J$ be differentiable at a point $x \in I$, and let $F \colon J \to \R$ be differentiable at $g(x)$. Then the composite function $f = F \circ g$ is differentiable at $x$, and
  \[
    f'(x) = F'(g(x)) \cdot g'(x).
  \]
  In Leibniz's notation, this may be written as
  \[
...
\begin{proof}
  Using the linear approximation from Proposition~1.1, we have $g(x + h) = g(x) + h\, g'(x) + o(h)$ as $h \to 0$. Set $k = g(x+h) - g(x) = h\, g'(x) + o(h)$. Since $F$ is differentiable at $g(x)$, we have
  \[
    F(g(x) + k) = F(g(x)) + k\, F'(g(x)) + o(k) \quad \text{as } k \to 0.
  \]
  Note that $k \to 0$ as $h \to 0$, and $o(k) = o(h)$ since $k = O(h)$. Therefore,
  \begin{align*}
    f(x + h) &= F(g(x + h)) = F(g(x) + k) \\
    &= F(g(x)) + k\, F'(g(x)) + o(h) \\
    &= F(g(x)) + (h\, g'(x) + o(h))\, F'(g(x)) + o(h) \\
    &= f(x) + h\, g'(x)\, F'(g(x)) + o(h).
  \end{align*}
  By the characterisation of differentiability, $f'(x) = g'(x)\, F'(g(x))$.
\end{proof}

\begin{thm}[Product rule]
  Let $I \subset \R$ be an open interval, and let $u, v \colon I \to \R$ be functions differentiable at a point $x \in I$. Then the product $f = uv$ is differentiable at $x$, and
  \[
    f'(x) = u'(x)\, v(x) + u(x)\, v'(x).
  \]
\end{thm}

The product rule generalises to higher derivatives as follows.

\begin{thm}[Leibniz's formula]
  Let $I \subset \R$ be an open interval, and let $u, v \colon I \to \R$ be functions that are $n$ times differentiable at a point $x \in I$, where $n \in \N$. Then the product $f = uv$ is $n$ times differentiable at $x$, and
  \[
    f^{(n)}(x) = \sum_{k = 0}^{n} \binom{n}{k} u^{(k)}(x)\, v^{(n - k)}(x),
  \]
  where by convention $u^{(0)} = u$ and $v^{(0)} = v$.
\end{thm}

\subsection{Taylor's theorem}
Taylor's theorem provides a polynomial approximation to a function in terms of its derivatives at a given point. This is one of the most fundamental results in analysis.

\begin{thm}[Taylor's theorem]
  Let $I \subset \R$ be an open interval, let $x_0 \in I$, and let $n \in \N$. Suppose that $f \colon I \to \R$ is $n$ times differentiable at $x_0$. Then
  \[
    f(x_0 + h) = \sum_{k=0}^{n} \frac{h^k}{k!} f^{(k)}(x_0) + R_n(h),
  \]
  where the remainder term satisfies $R_n(h) = o(h^n)$ as $h \to 0$. If, moreover, $f$ is $(n+1)$ times differentiable at $x_0$, then $R_n(h) = O(h^{n+1})$ as $h \to 0$.
\end{thm}

\begin{rem}
  This theorem provides only a \emph{local} approximation of $f$ near $x_0$; it does not, in general, yield information about the behaviour of $f$ far from $x_0$.
\end{rem}

An equivalent formulation is obtained by substituting $x = x_0 + h$:
\[
  f(x) = \sum_{k=0}^{n} \frac{(x - x_0)^k}{k!} f^{(k)}(x_0) + R_n(x - x_0).
\]
The polynomial $\displaystyle T_n(x) = \sum_{k=0}^{n} \frac{(x - x_0)^k}{k!} f^{(k)}(x_0)$ is called the \emph{Taylor polynomial of order $n$} of $f$ at $x_0$.

\begin{defi}[Taylor series]
  Let $f$ be a function that is infinitely differentiable at a point $x_0$. The \emph{Taylor series} of $f$ at $x_0$ is the formal power series
  \[
    \sum_{k=0}^{\infty} \frac{(x - x_0)^k}{k!} f^{(k)}(x_0).
  \]
  When $x_0 = 0$, this is also called the \emph{Maclaurin series} of $f$.
\end{defi}

\begin{rem}
  The Taylor series of $f$ need not converge for all $x$, and even when it converges, it need not converge to $f(x)$. A function whose Taylor series converges to itself on an interval is said to be \emph{analytic} on that interval.
\end{rem}

\subsection{L'H\^opital's rule}
L'H\^opital's rule is a useful technique for evaluating limits of indeterminate forms such as $\frac{0}{0}$ or $\frac{\infty}{\infty}$.

\begin{thm}[L'H\^opital's rule]
  Let $I \subset \R$ be an open interval containing a point $x_0$, and let $f, g \colon I \setminus \{x_0\} \to \R$ be differentiable functions. Suppose that:
  \begin{enumerate}
    \item $\displaystyle \lim_{x \to x_0} f(x) = \lim_{x \to x_0} g(x) = 0$;
    \item $g'(x) \neq 0$ for all $x \in I \setminus \{x_0\}$;
    \item the limit $\displaystyle \lim_{x \to x_0} \frac{f'(x)}{g'(x)}$ exists (possibly equal to $\pm\infty$).
  \end{enumerate}
  Then
  \[
...
\begin{proof}
  Assume that $f$ and $g$ extend continuously to $x_0$ with $f(x_0) = g(x_0) = 0$, and that they are differentiable at $x_0$ with $g'(x_0) \neq 0$. By the linear approximation (Proposition~1.1), we have
  \begin{align*}
    f(x) &= f(x_0) + (x - x_0)\, f'(x_0) + o(x - x_0) = (x - x_0)\, f'(x_0) + o(x - x_0), \\
    g(x) &= g(x_0) + (x - x_0)\, g'(x_0) + o(x - x_0) = (x - x_0)\, g'(x_0) + o(x - x_0).
  \end{align*}
  Dividing and taking the limit:
  \[
    \lim_{x \to x_0} \frac{f(x)}{g(x)} = \lim_{x \to x_0} \frac{f'(x_0) + \frac{o(x-x_0)}{x-x_0}}{g'(x_0) + \frac{o(x-x_0)}{x-x_0}} = \frac{f'(x_0)}{g'(x_0)}.
  \]
  If $f'$ and $g'$ are continuous at $x_0$, then $\displaystyle \lim_{x \to x_0} \frac{f'(x)}{g'(x)} = \frac{f'(x_0)}{g'(x_0)}$, which gives the result. The general case requires a more careful argument using Cauchy's mean value theorem.
\end{proof}

\begin{rem}
  The rule can be applied repeatedly if the resulting limit is again an indeterminate form, provided the hypotheses remain satisfied at each stage.
\end{rem}

\section{Integration}
\subsection{Integration}
\begin{defi}[Integral]
  Let $f$ be a function defined on an interval $[a, b]$. The \emph{integral} of $f$ over $[a, b]$ is defined as the limit of a Riemann sum:
  \[
    \int_a^b f(x) \;\d x = \lim_{\Delta x\to 0}\sum_{n=0}^N f(x_n)\Delta x,
  \]
  where $\Delta x = \frac{b-a}{N}$ is the width of each subinterval and $x_n = a + n\Delta x$ are the sample points. More generally, the term ``integral'' refers to any limit of such sums (the precise definition of the Riemann integral is treated in Analysis I). Pictorially, this represents the area under the curve:
  \begin{center}
    \begin{tikzpicture}
...
The area of each rectangle from $x_n$ to $x_{n+1}$ is $f(x_n)\Delta x + O(\Delta x^2)$. For a differentiable function $f$, the total area under the graph from $a$ to $b$ is therefore
\[
...
\begin{thm}[Fundamental Theorem of Calculus]
  Let $f$ be a continuous function on $[a, b]$, and define $F(x) = \int_a^x f(t)\;\d t$. Then $F$ is differentiable and $F'(x) = f(x)$.
\end{thm}
...
\begin{proof}
  ...
  \end{align*}
\end{proof}
As immediate consequences, if $f$ is continuous and $g$ is differentiable, then
\[
...
Integration is substantially harder than differentiation. While the product rule and chain rule, combined with a table of standard derivatives, suffice to differentiate most expressions, many seemingly simple integrands can be surprisingly difficult---or even impossible---to integrate in closed form. Several techniques are available to evaluate integrals.
...
\begin{proof}
  By the product rule, $(uv)' = uv' + u'v$. Integrating both sides and rearranging yields the result.
\end{proof}

\begin{eg}[$xe^{-x}$ improper integral]
  Consider $\int_0^\infty xe^{-x}\d x$. Let $u = x$ and $v' = e^{-x}$. Then $u' = 1$ and $v = -e^{-x}$. We have
  \begin{align*}
...
The preceding sections dealt with functions of a single variable. We now consider functions of several variables, such as $f(x, y)$, which can be differentiated with respect to each variable independently.
\begin{defi}[Partial derivative]
  Let $f(x, y)$ be a function of two variables. The \emph{partial derivative} of $f$ with respect to $x$, denoted $\left. \frac{\partial f}{\partial x}\right|_y$ or simply $\frac{\partial f}{\partial x}$, is the rate of change of $f$ as $x$ varies while $y$ is held constant:
  \[
...
A common shorthand notation is as follows.
\begin{notation}
  \[
    f_x = \frac{\partial f}{\partial x},\quad f_{xy} = \frac{\partial^2 f}{\partial y\partial x}.
  \]
\end{notation}
Note that $f_{xy}$ means $(f_x)_y$: first differentiate with respect to $x$, then with respect to $y$. However, under mild regularity conditions, the order of differentiation does not matter.
\begin{thm}[Schwarz's theorem]
  Let $f$ be a function with continuous second partial derivatives. Then $f_{xy} = f_{yx}$.
\end{thm}
The proof of this theorem is beyond the scope of this course; we shall take it as given.

\subsection{Chain rule}
Let $f(x, y)$ be a differentiable function of two variables. Consider a displacement $(x, y) \to (x+\delta x, y + \delta y)$. The change in $f$ is
\begin{align*}
...
\end{align*}
Taking the limit as $\delta x, \delta y \to 0$ yields the following result.
\begin{thm}[Chain rule for partial derivatives]
  Let $f(x, y)$ be a differentiable function. Then
  \[
    \d f = \frac{\partial f}{\partial x}\d x + \frac{\partial f}{\partial y}\d y.
  \]
  This differential form can be integrated:
  \[
    \int \d f = \int \frac{\partial f}{\partial x}\d x + \int \frac{\partial f}{\partial y}\d y.
  \]
  Alternatively, if $x$ and $y$ are functions of a parameter $t$, dividing by $\d t$ gives
  \[
    \frac{\d f}{\d t} = \frac{\partial f}{\partial x}\frac{\d x}{\d t} + \frac{\partial f}{\partial y}\frac{\d y}{\d t}.
  \]
\end{thm}

If $t$ is taken to be the arclength $s$ along a curve, then
\[
...
Alternatively, if the path is given explicitly as $y = y(x)$, then $f = f(x, y(x))$ and
\[
  \frac{\d f}{\d x} = \left.\frac{\partial f}{\partial x}\right|_y + \frac{\partial f}{\partial y}{\frac{\d y}{\d x}}.
\]
The chain rule also applies to changes of variables. For example, under a change to polar coordinates where $x = x(r, \theta)$ and $y = y(r, \theta)$,
\[
...
Consider a function $F(x, y, z)$ and a level surface defined by $F(x, y, z) = c$ for some constant $c$. This equation implicitly defines $z$ as a function of $x$ and $y$, written $z = z(x, y)$. For example, if $F(x, y, z) = xy^2 + yz^2 + z^5x = 5$, then $z$ cannot be expressed explicitly as a function of $x$ and $y$ (doing so would require solving a quintic equation). Nevertheless, the partial derivatives of $z(x, y)$ can be computed by differentiating the equation $F(x, y, z) = c$ with respect to $x$ while holding $y$ constant:
\begin{align*}
...
\begin{thm}[Multi-variable implicit differentiation]
  Let $F(x, y, z)$ be a differentiable function, and suppose $z = z(x, y)$ is implicitly defined by
  \[
    F(x, y, z) = c
  \]
  for some constant $c$. If $\frac{\partial F}{\partial z} \neq 0$, then
  \[
...
Consider a function $f(x, c)$ depending on a variable $x$ and a parameter $c$. Define the integral $I(b, c) = \int_0^b f(x, c)\,\d x$. By the Fundamental Theorem of Calculus, $\frac{\partial I}{\partial b} = f(b, c)$. To find $\frac{\partial I}{\partial c}$, observe that
\begin{align*}
...
\end{align*}
More generally, suppose both the upper limit and a parameter in the integrand depend on $x$: let $I(x) = \int_0^{b(x)} f(y, c(x))\,\d y$, where $b$ and $c$ are differentiable functions of $x$. By the chain rule,
\[
...
\begin{thm}[Differentiation under the integral sign]
  Let $f(y, c)$ be continuously differentiable, and let $b(x)$ and $c(x)$ be differentiable functions. Then
  \[
    \frac{\d}{\d x} \int_{0}^{b(x)} f(y, c(x))\;\d y= f(b(x), c(x))\,b'(x) + c'(x)\int_0^{b(x)} \frac{\partial f}{\partial c}\;\d y.
  \]
\end{thm}
This technique is useful for evaluating certain integrals that would otherwise be difficult to compute.

\begin{eg}[Gaussian parameter derivative]
...
A differential equation is an equation involving derivatives of an unknown function. For example, $x^2\frac{\d y}{\d x} + 2y = 0$ is a differential equation in the unknown function $y(x)$. Unlike algebraic equations where the solution is a number, the solution to a differential equation is a function satisfying the equation. This chapter focuses on \emph{first-order differential equations}, in which only first derivatives appear.
...
\begin{defi}[Eigenfunction]
  An \emph{eigenfunction} of the differential operator is a function whose functional form is unchanged by differentiation; only its magnitude changes. That is, $f$ is an eigenfunction if
  \[
    \frac{\d f}{\d x} = \lambda f
  \]
  for some constant $\lambda$.
\end{defi}
\begin{eg}[Exponential eigenfunction]
...
\begin{thm}[Exponential solutions]
...
\begin{eg}[First-order constant coefficients]
...
A homogeneous equation, such as $5y' - 3y = 0$, has no terms independent of $y$. A \emph{forced} (or \emph{inhomogeneous}) equation contains additional terms that do not involve the dependent variable. For example, $5y' - 3y = 10$ and $5y' - 3y = x^2$ are forced equations, with $10$ and $x^2$ called the \emph{forcing terms}.
...
\]
where $P$ and $Q$ are functions of $x$ and $y$. (More general forms involving powers of $y'$ exist but are not considered here.)
...
\begin{defi}[Separable equation]
  ...
  \]
\end{defi}
Separable equations are often the simplest to solve.
...
\end{defi}
Equilibrium points are typically easy to find: given an expression for $\frac{\d y}{\d t}$, one simply solves $\frac{\d y}{\d t} = 0$.
...
The objective of this section is to study how to find equilibrium points and determine their stability.
...
In many physical systems, the governing laws do not change with time, so $\dot y$ depends only on $y$ itself. Such systems are called \emph{autonomous}.
...
Consider $\ddot y + \omega_0^2 y = \sin \omega_0 t$. The complementary solution is $y_c = A\sin \omega_0 t + B\cos \omega_0 t$. We notice that the forcing is itself a complementary function. So if we guess a particular integral $y_p = C\sin \omega_0 t + D\cos \omega_0 t$, we'll simply find $\ddot y_p + \omega_0 ^2 y_p = 0$, so we can't balance the forcing.
...
When a differential equation cannot be solved in closed form, it is often possible to find a solution expressed as a Taylor series.
...
The Frobenius series can alternatively be written as
\begin{align*}
...
The proofs of these results are beyond the scope of this text; the focus here is on applying them.
...
\begin{eg}[Frobenius with half-integer index]
  Consider $4xy'' + 2(1 - x^2)y' - xy = 0$. Note that $x = 0$ is a singular point. Multiplying the equation by $x$ yields the equidimensional form
  \[
...
\begin{enumerate}
  ...
  \item If $\sigma_2 - \sigma_1$ is an integer (including when they are equal), there is one solution of the form
    ...
    In this case, $\sigma = \sigma_1$ typically does not yield a valid solution, as demonstrated in the following example. Instead, the second solution is generally of the form
    \[
      y_2 = \ln(x - x_0)y_1 + \sum_{n = 0}^\infty b_n(x - x_0)^{n + \sigma_1}.
    \]
    This form arises from resonance between the two solutions. However, if the resonance condition is not triggered, it is possible for two regular Frobenius series solutions to exist.
...
\begin{defi}[Gradient vector]
  ...
  \]
\end{defi}
This is taken as the formal definition of $\nabla f$. The expression $\nabla f = \left(\frac{\partial f}{\partial x}, \frac{\partial f}{\partial y}\right)$ then follows as a theorem from this definition.
...
Then near $\mathbf{x}_0$, $f = $ constant $\Rightarrow \delta\mathbf{x}\cdot H\cdot \delta\mathbf{x} = $ constant, i.e.\ $\lambda_1 X^2 + \lambda_2 Y^2 = $ constant. At a maximum or minimum, $\lambda_1$ and $\lambda_2$ have the same sign. So these contours are locally ellipses. At a saddle point, they have different signs and the contours are locally hyperbolae.
